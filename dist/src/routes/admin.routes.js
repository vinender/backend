...
router.get('/stats', _adminmiddleware.authenticateAdmin, async (req, res)=>{
    try {
...
router.get('/booking-stats', _adminmiddleware.authenticateAdmin, async (req, res)=>{
    try {
        const { period = 'Today' } = req.query;
        const now = new Date();
        const startOfToday = new Date(now.getFullYear(), now.getMonth(), now.getDate());
...
router.get('/field-utilization', _adminmiddleware.authenticateAdmin, async (req, res)=>{
    try {
        const { period = 'Today' } = req.query;
        const now = new Date();
        const startOfToday = new Date(now.getFullYear(), now.getMonth(), now.getDate());
