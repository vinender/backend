...
class ReviewController {
    ...
    async getFieldReviews(req, res) {
        try {
            ...
            const [reviews, total] = await Promise.all([
                _database.default.fieldReview.findMany({
                    where,
                    skip,
                    take: limit,
                    orderBy,
                ...
                _database.default.fieldReview.count({
                    where
                })
            ]);
            // Get rating distribution
            const ratingDistribution = await _database.default.fieldReview.groupBy({
                by: [
                    'rating'
                ],
                where: {
            ...
            const avgRating = await _database.default.fieldReview.aggregate({
                where: {
                    fieldId
                },
                _avg: {
    ...
    async createReview(req, res) {
        try {
            ...
            const existingReview = await _database.default.fieldReview.findFirst({
                where: {
                    fieldId,
                    userId
                }
            ...
            const hasBooked = await _database.default.booking.findFirst({
                where: {
                    fieldId,
                    userId,
                    status: 'COMPLETED'
            ...
            // Create the review
            const review = await _database.default.fieldReview.create({
                data: {
                    fieldId,
                    userId,
                    userName: user?.name,
            ...
            const reviewStats = await _database.default.fieldReview.aggregate({
                where: {
                    fieldId
                },
                _avg: {
    ...
    async updateReview(req, res) {
        try {
            ...
            const review = await _database.default.fieldReview.findFirst({
                where: {
                    id: reviewId,
                    userId
                }
            ...
            // Update the review
            const updatedReview = await _database.default.fieldReview.update({
                where: {
                    id: reviewId
                },
                data: {
    ...
    async deleteReview(req, res) {
        try {
            ...
            const review = await _database.default.fieldReview.findUnique({
                where: {
                    id: reviewId
                }
            });
            ...
            // Delete the review
            await _database.default.fieldReview.delete({
                where: {
                    id: reviewId
                }
            });
    ...
    async markHelpful(req, res) {
        try {
            ...
            const review = await _database.default.fieldReview.update({
                where: {
                    id: reviewId
                },
                data: {
...
const _default = new ReviewController();
