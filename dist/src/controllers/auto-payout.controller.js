...
class AutoPayoutController {
   ...
   */ getPayoutSummary = (0, _asyncHandler.asyncHandler)(async (req, res, next)=>{
        ...
        const summary = await _autopayoutservice.automaticPayoutService.getPayoutSummary(userId);
        res.json({
            success: true,
            data: summary
        });
   ...
   */ processBookingPayout = (0, _asyncHandler.asyncHandler)(async (req, res, next)=>{
        ...
        const payout = await _autopayoutservice.automaticPayoutService.processBookingPayoutAfterCancellationWindow(bookingId);
        if (!payout) {
            return res.json({
                success: false,
   ...
   */ processRefundWithFees = (0, _asyncHandler.asyncHandler)(async (req, res, next)=>{
        ...
        const refund = await _autopayoutservice.automaticPayoutService.processRefundWithFeeAdjustment(bookingId, reason || 'Customer requested refund');
        res.json({
            success: true,
