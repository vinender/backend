...
class FieldController {
    ...
    createField = (0, _asyncHandler.asyncHandler)(async (req, res, next)=>{
        ...
        const existingFields = await _fieldmodel.default.findByOwner(ownerId);
        if (existingFields && existingFields.length > 0) {
            throw new _AppError.AppError('Field owners can only have one field. Please update your existing field instead.', 400);
        }
        ...
        if (req.body.openingTime && req.body.closingTime) {
            ...
            if (diffHours < minimumHours) {
                throw new _AppError.AppError(`Field must be open for at least ${minimumHours} hours`, 400);
            }
        }
        const fieldData = {
            ...req.body,
    ...
    saveFieldProgress = (0, _asyncHandler.asyncHandler)(async (req, res, next)=>{
        ...
        if (allStepsCompleted && !field.isActive) {
            await _fieldmodel.default.update(fieldId, {
                isActive: true
            });
        }
        res.json({
    ...
    getFieldBookings = (0, _asyncHandler.asyncHandler)(async (req, res, next)=>{
        ...
        try {
            ...
            const totalEarnings = await _database.default.booking.aggregate({
                where: {
                    fieldId,
                    status: 'COMPLETED'
                },
    ...
    getPreviousBookings = (0, _asyncHandler.asyncHandler)(async (req, res, next)=>{
        const ownerId = req.user.id;
        const { page = 1, limit = 12 } = req.query;
        try {
            // First get the owner's field
