...
class PaymentController {
    ...
    async createPaymentIntent(req, res) {
        try {
            ...
            let paymentIntent;
            try {
                paymentIntent = await _stripeconfig.stripe.paymentIntents.create(paymentIntentParams, {
            ...
            const systemSettings = await _database.default.systemSettings.findFirst();
            ...
            if (paymentIntent.status === 'succeeded') {
                ...
                if (payoutReleaseSchedule === 'immediate' && fieldOwnerStripeAccount && fieldOwnerStripeAccount.chargesEnabled && fieldOwnerStripeAccount.payoutsEnabled) {
                    try {
                        ...
                        const transfer = await _stripeconfig.stripe.transfers.create({
                            amount: Math.round(fieldOwnerAmount * 100),
                            currency: 'gbp',
    ...
    async confirmPayment(req, res) {
        try {
            ...
            if (paymentIntent.status === 'succeeded') {
                ...
                const { fieldOwnerAmount, platformFeeAmount, commissionRate } = await (0, _commissionutils.calculatePayoutAmounts)(booking.totalPrice, field?.ownerId || '');
                    ...
                    await _database.default.$transaction(async (tx)=>{
                        ...
                        if (!booking) {
                            ...
                            if (metadata.userId && metadata.fieldId && metadata.date && metadata.timeSlot) {
                                ...
                                // Calculate commission amounts
                                const payoutAmounts = await (0, _commissionutils.calculatePayoutAmounts)(paymentIntent.amount / 100, field?.ownerId || '');
                                // Create transaction record with commission details
