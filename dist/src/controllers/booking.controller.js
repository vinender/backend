...
class BookingController {
    ...
    createBooking = (0, _asyncHandler.asyncHandler)(async (req, res, next)=>{
        ...
        // Check availability
        const isAvailable = await _bookingmodel.default.checkAvailability(fieldId, new Date(date), startTime, endTime);
        if (!isAvailable) {
            throw new _AppError.AppError('This time slot is not available', 400);
        }
    ...
    getBooking = (0, _asyncHandler.asyncHandler)(async (req, res, next)=>{
        ...
        // Check access rights
        const hasAccess = userRole === 'ADMIN' || booking.userId === userId || booking.field.ownerId === userId;
        if (!hasAccess) {
    ...
    updateBookingStatus = (0, _asyncHandler.asyncHandler)(async (req, res, next)=>{
        ...
        if (status === 'CONFIRMED') {
            ...
            try {
                ...
                await _payoutservice.payoutService.processBookingPayout(id);
    ...
    cancelBooking = (0, _asyncHandler.asyncHandler)(async (req, res, next)=>{
        ...
        if (isRefundEligible && isDogOwner) {
            try {
                refundResult = await _refundservice.default.processRefund(id, reason);
            } catch (refundError) {
    ...
    updateBooking = (0, _asyncHandler.asyncHandler)(async (req, res, next)=>{
        ...
        if (date || startTime || endTime) {
            ...
            const isAvailable = await _bookingmodel.default.checkAvailability(booking.fieldId, newDate, newStartTime, newEndTime, id // Exclude current booking from check
            );
            if (!isAvailable) {
                throw new _AppError.AppError('The new time slot is not available', 400);
            }
    ...
    getMyRecurringBookings = (0, _asyncHandler.asyncHandler)(async (req, res, next)=>{
        ...
        const [subscriptions, recurringBookings] = await Promise.all([
            ...
            _database.default.booking.findMany({
                where: {
                    ...
                    status: status === 'active' ? 'CONFIRMED' : {
                        in: [
                            'CANCELLED',
                            'COMPLETED'
                        ]
        ...
        const formattedRecurringBookings = recurringBookings.map((booking)=>({
                ...
                interval: booking.repeatBooking,
                dayOfWeek: null,
                dayOfMonth: null,
