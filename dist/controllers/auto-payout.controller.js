...
class AutoPayoutController {
    ...
    getPayoutSummary = (0, asyncHandler_1.asyncHandler)(async (req, res, next) => {
        ...
        const summary = await auto_payout_service_1.automaticPayoutService.getPayoutSummary(userId);
        res.json({
            success: true,
            data: summary
        });
    ...
    processBookingPayout = (0, asyncHandler_1.asyncHandler)(async (req, res, next) => {
        ...
        const payout = await auto_payout_service_1.automaticPayoutService.processBookingPayoutAfterCancellationWindow(bookingId);
        if (!payout) {
            return res.json({
                success: false,
    ...
    processRefundWithFees = (0, asyncHandler_1.asyncHandler)(async (req, res, next) => {
        ...
        const refund = await auto_payout_service_1.automaticPayoutService.processRefundWithFeeAdjustment(bookingId, reason || 'Customer requested refund');
        res.json({
            success: true,
