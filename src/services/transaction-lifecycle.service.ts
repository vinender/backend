...
export class TransactionLifecycleService {
   ...
   */
  async updateFundsPending(stripePaymentIntentId: string, balanceTransactionId?: string) {
    try {
   ...
   */
  async updateFundsAvailable(stripePaymentIntentId: string) {
    try {
      const transaction = await prisma.transaction.findFirst({
        where: { stripePaymentIntentId }
      });
   ...
   */
  async updatePayoutCompleted(stripePayoutId: string) {
    try {
      const transaction = await prisma.transaction.findFirst({
   ...
   */
  async getTransactionByPaymentIntent(stripePaymentIntentId: string) {
    try {
      const transaction = await prisma.transaction.findFirst({
        where: { stripePaymentIntentId },
        include: {
